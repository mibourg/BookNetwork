...
export class BrowsePeopleScreen extends React.Component {
  ...
  fetchDisplayedUsers() {
    firebase.database().ref("users/").once('value').then((userListSnapshot) => {
      userListSnapshot.forEach((userSnapshot) => {
        if (userSnapshot.val().id !== firebase.auth().currentUser.id) {
  ...
  renderUser(data, index) {
    const item = data;
    return <UserListItem addUserToFriendsList={() => this.addUserToFriendsList(item)} removeUserFromFriendsList={() => this.removeUserFromFriendsList(item)} browsePeopleScreen={true} id={item.id} profilePhotoURL={item.profilePictureURL} name={item.name} bio={item.bio} openProfileModal={() => this.openProfileModal(item)}/>
  }
        ...
        <View>
          ...
          <ScrollView>
            <View style={styles.usersContainer}>
              <CompleteFlatList searchKey={['name', 'bio']} keyExtractor={(item, index) => String(index)} style={styles.userList} data={this.state.displayedUsers.slice(0,this.state.displayedUsers.length)} renderItem={this.renderUser}/>
              <View style={{height: 1, backgroundColor: "#808080"}}></View>
            </View>
          </ScrollView>
        </View>
